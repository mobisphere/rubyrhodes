...
public class Rhoelements implements Runnable {
	
	private static final RhoLogger LOG = RhoLogger.RHO_STRIP_LOG ? new RhoEmptyLogger() : 
		new RhoLogger("Rhoelements");

	public static RubyModule RhoelementsModule;    
	...
	public void run() {
		  LOG.INFO("$$$ Register Rhoelements Ruby class");
   	  	  // register Ruby Module
          RhoelementsModule = RubyAPI.defineModule("Rhoelements");        
          // register Ruby method
          RhoelementsModule.getSingletonClass().defineMethod("native_process_string", new RubyOneArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg0, RubyBlock block) {
				if ( arg0 instanceof RubyString )
				{
					...
					return ObjectFactory.createString(result);
					  ...
					  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Rhoelements.native_process_string: wrong argument type.Should be String");
				}
			}
          });
          RhoelementsModule.getSingletonClass().defineMethod("calc_summ", new RubyTwoArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg1, RubyValue arg2, RubyBlock block) {
				try {
						...
						if (arg1 instanceof RubyInteger) {
							...
							if (arg1 instanceof RubySymbol) {
								...
								x = rv.toInt();
								  ...
								  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Rhoelements.calc_summ: wrong argument type.Should be Fixnum or Integer");
							}
						}
					}
					...
					e.printStackTrace();
					LOG.ERROR("Rhoelements.calc_summ failed with exception", e);
